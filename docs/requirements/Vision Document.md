...
verifies encrypted credentials stored in its own SQLite database instead.
...
| **F-09** | Pluggable authentication | Authenticate the single owner via either an external JWT service or local encrypted-login credentials, selected at startup; authorize every operation. |

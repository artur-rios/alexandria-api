...
5. For the subtypes that carry embedded metadata — audio, image, document, video, and comic — the system reads the file's own metadata and prefills the subtype row with it (FR-FC-25). Extraction is **best-effort**: an unreadable or metadata-less file simply leaves the fields empty, and the owner can set or correct any of them afterwards via UC-04. The row is stamped with the extraction that was applied to it, which is what lets UC-02 tell a row written by an older reading from a current one. A file that gave up nothing is stamped too: the stamp records which extraction has been applied, not which one succeeded, and re-opening an unreadable file on every later pass would buy only a retry that a later extraction — which moves the version, and so revisits every row — performs anyway.
...
3. For each path whose size or modification time differs from what the record holds, the system treats the file as changed: it stores the new size and modification time, updates `indexedAt`, and clears the record's content hash, so a hash cannot outlive the bytes it described (FR-FC-10). A path whose size and modification time both match is recorded unchanged.
4. For each unchanged path whose row an **older extraction** wrote (UC-01 step 5's stamp), the system reads the file's own metadata again and fills the subtype fields the catalog is missing, then stamps the row (FR-FC-25). The run's own outcome carries how many were filled, and its log line names the number; the persisted per-run tally (FR-FC-27) keeps its four counts, because a file whose metadata was re-read is still a file that did not change. It **fills and never replaces**: a field the owner set via UC-04, and a field an earlier extraction already wrote, are both left exactly as they are. This is what closes a gap a re-index could not close before — a library indexed before the extractor could read a field held nothing there for the rest of its life, because a re-index never read tags and an index skips a path it already holds.
...
| ID | Condition | Outcome |
| --- | --- | --- |
| AF-01 | A path on disk was deleted since indexing | The system sets `missingAt` (per main flow step 5); the record is neither deleted nor moved to the `deleted` state. (Explicit deletion is UC-06.) |
| AF-02 | The caller is not authenticated | The system denies with an unauthorized error. |

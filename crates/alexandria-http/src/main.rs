...
async fn main() -> Result<()> {
    ...
    if settings.auth.mode == AuthMode::Windows {
        ...
        // FR-AU-24: Windows mode proves the process was launched by the owner,
        // not who is calling it — so any caller that can reach the port is
        // authorized. On loopback that is the owner's own machine. Anywhere
        // else it is the network, and the operator should hear about it.
